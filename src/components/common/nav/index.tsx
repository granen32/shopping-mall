...
import WLOGO from "@/assets/img/header/w-logo.svg";
...
const Nav = ({ type, onClick }: NavProps) => {
  ...
  const handleScroll = () => {
    ...
    logoImage.src = isScrolled || !isHome ? LOGO : WLOGO;
    navElement.classList.toggle("text-white", !isScrolled && isHome);
    navElement.classList.toggle("text-black", isScrolled || !isHome);
    navElement.classList.toggle("border-b-[1px]", isScrolled || !isHome);
    navElement.classList.toggle("bg-white", isScrolled || !isHome);
    svgElements.forEach((svg) => {
      svg.style.stroke = isScrolled || !isHome ? "#000" : "#fff";
    });
  };
  ...
  return (
    <>
      ...
      <nav
        id="nav-element"
        ref={headerRef}
        className="text-white font-medium fixed left-0 top-[40px] w-full z-[20] transition-colors duration-300 ease-in-out">
        <div className="wrap-ml relative z-10 max-w-[1680px] w-full mx-auto">
      ...
      <Modal visible={visible} onClose={closeModal} className="modal-root">
        <SearchInput />
      </Modal>
    </>
  );
